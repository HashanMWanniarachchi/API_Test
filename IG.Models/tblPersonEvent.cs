//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace IG.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class tblPersonEvent
    {
        public System.Guid fldID { get; set; }
        public System.DateTime fldDateCreated { get; set; }
        public System.DateTime fldDateUpdated { get; set; }
        public Nullable<System.Guid> fldUserId { get; set; }
        public byte fldActive { get; set; }
        public Nullable<System.Guid> fldPersonID { get; set; }
        public int fldEventNumber { get; set; }
        public string fldEventName { get; set; }
        public Nullable<System.Guid> fldEventType { get; set; }
        public Nullable<System.Guid> fldFocus { get; set; }
        public Nullable<System.DateTime> fldDate { get; set; }
        public string fldVenue { get; set; }
        public string fldAddress { get; set; }
        public Nullable<System.Guid> fldSuburb { get; set; }
        public Nullable<int> fldParticipants { get; set; }
        public Nullable<System.DateTime> fldStartTime { get; set; }
        public Nullable<System.DateTime> fldEndTime { get; set; }
        public Nullable<double> fldAdditionalTime { get; set; }
        public string fldComments { get; set; }
    }
}
